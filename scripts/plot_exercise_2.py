...
sim_time_list = np.load(f'../data/sim_time_list.npy')
rho_list = np.load(f'../data/rho_list.npy')
...
for i, rho in enumerate(rho_list):
    ...
    axes[i].fill_between(sim_time_list, np.take(np.array(upper_dict[rho]),0, axis = 1),np.take(np.array(lower_dict[rho]),0, axis = 1),alpha=0.5)
    axes[i].fill_between(sim_time_list, np.take(np.array(upper_dict[rho]),1, axis = 1),np.take(np.array(lower_dict[rho]),1, axis = 1),alpha=0.5)
    axes[i].fill_between(sim_time_list, np.take(np.array(upper_dict[rho]),2, axis = 1),np.take(np.array(lower_dict[rho]),2, axis = 1),alpha=0.5)

    axes[i].text(1.05,0.5, horizontalalignment='center', verticalalignment='center', s = f'rho = {rho}',transform=axes[i].transAxes, rotation = 45)
